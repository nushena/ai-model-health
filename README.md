...
- 🎯 **智能状态识别**: 四种状态（成功/失败/等待检测/已离线），精准反映模型可用性
- 🧹 **自动数据清理**: 每天凌晨 3 点自动清理过期数据，连续 15 次离线自动删除模型记录
...
CREATE TABLE batches (
  ...
  is_completed INTEGER DEFAULT 0 -- 是否完成检测 (0/1)
);
...
- 采用"完成后等待"模式，避免检测重叠
- 服务启动后立即执行第一次检测
- **批次完成标记**: 所有模型检测完成后标记批次为已完成
- **自动清理任务**: 每天凌晨 3 点清理过期数据
- **离线模型清理**: 检测完成后自动删除连续 15 次离线的模型
